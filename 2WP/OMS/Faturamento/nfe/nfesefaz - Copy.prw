...
±±³ Hist Atualizações - Fonte baixado em 02/06/2017 com data 09/05/2017   ³±±
±±³ Hist Atualizações - Fonte baixado em 08/05/2018 com data 18/01/2018,  ³±±
±±³    alteração efetuada para contemplar a NFe 4.0                       ³±±
±±³ Hist Atualizações - Fonte baixado em 13/07/2018 com data 10/07/2018,  ³±±
±±³ Hist Atualizações - Fonte baixado em xx/xx/xxxx com data xx/xx/xxxx   ³±±
±±³  *************************************************************        ³±±
±±³  **  As customizações do fonte procure pela palavra "sobel" **        ³±±
...
Private lDifal	:= .F.
Private lDifer		:= .F.
IIF(lMvNFLeiZF == ".T.",lMvNFLeiZF := .T.,lMvNFLeiZF := .F.) // CUSTOMIZAÇÃO SOBEL - A VARIAVEL lMvNFLeiZF ESTA VINDO CARACTER QUANDO DEVERIA SER LOGICA
...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
				...
				If !Empty(SF2->F2_DUPL)	
					...
					#IFDEF TOP
						...
						BeginSql Alias cAliasSE1
							...
							SELECT E1_FILIAL,E1_PREFIXO,E1_NUM,E1_PARCELA,E1_TIPO,E1_VENCORI,E1_VALOR,E1_VLCRUZ,E1_ORIGEM,E1_PIS,E1_COFINS,E1_CSLL,E1_INSS,E1_VLRREAL,E1_IRRF,E1_ISS,E1_ZZPORT  //SOBEL CUSTON-PARA ACHAR A FORMA DE PAGAMENTO
							FROM %Table:SE1% SE1
							WHERE
						...
						SF2->F2_DOC == (cAliasSE1)->E1_NUM     
							If (cAliasSE1)->E1_TIPO = MVNOTAFIS .OR. (cAliasSE1)->E1_TIPO = 'DP' .OR. ((Alltrim((cAliasSE1)->E1_ORIGEM) $ 'LOJA701|FATA701|LOJA010') .AND. (cAliasSE1)->E1_TIPO $ cWhere)
								...
								If cValLiqB == "2" // 1 Baixa - 2 Emissao
									...
									,(nValDupl-(cAliasSE1)->E1_PIS-(cAliasSE1)->E1_COFINS-(cAliasSE1)->E1_CSLL-(cAliasSE1)->E1_INSS)-(cAliasSE1)->E1_IRRF-(cAliasSE1)->E1_ISS,(cAliasSE1)->E1_ZZPORT}) //SOBEL CUSTOM FORMA DE PGTO										
								Else
							...
							If (cAliasSD2)->D2_TIPO $ "DBN"
								...
								If ( MsSeek(xFilial("SD1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+PADL(alltrim((cAliasSD2)->D2_ITEMORI),TamSx3("D2_ITEMORI")[1],"0")) ) .OR. ;
									...
									if AllTrim(SF1->F1_ESPECIE)<>"NFP"
										...
										//***Personalização SOBEL -- José Carlos e Daniel Tornisielo - Exxemplo TI - 10/04/2015
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
						...
						//Coloco o numero do Romaneio na mensagem para nota - 2WPTECH 
						DbSelectArea("SC9")
						DbSetOrder(1)
						DbSeek(xFilial("SC9")+SC5->C5_NUM )
						If !Empty(SC9->C9_ROMANEI)
						   cRomaneio := "Romaneio: "+SC9->C9_ROMANEI
						   If !cRomaneio $  cMensFis
						      cMensFis += cRomaneio
						   EndIf 
						EndIf  
						...
						//Customização Sobel
						//Caso for emitir nota fiscal de licitação, comentar o padrao e habilitar o custom
						...
						//CUSTOM inicio
						/*
						If SC6->(FieldPos("C6_NUMPCOM")) > 0 
							If !Empty(SC6->C6_NUMPCOM) 
								aadd(aPedCom,{SC6->C6_NUMPCOM,SC6->C6_ITEM})
							Else
								aadd(aPedCom,{})
							EndIf
						Else
						...
						If lNfCup .Or. (cAliasSD2)->D2_ORIGLAN $ "VD|LO"
							...
							EndIf
			            Else 
						//***Personalização SOBEL -- José Carlos e Daniel Tornisielo - Exxemplo TI - 01/04/2015
						If AllTrim(SC5->C5_MENPAD) == "053" //LICITAÇÃO
							nDesconto := (cAliasSD2)->D2_DESCON            	
						Else  
						  	nDesconto := 0
						EndIf  	
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							SB1->B1_UM,;
							(cAliasSD2)->D2_QUANT,;
							IIF(!(cAliasSD2)->D2_TIPO$"IP",IIF(!(lMvNFLeiZF),(cAliasSD2)->D2_TOTAL+(cAliasSD2)->D2_DESCZFR,(cAliasSD2)->D2_TOTAL+(cAliasSD2)->D2_DESCZFR - ((cAliasSD2)->D2_DESCZFP+(cAliasSD2)->D2_DESCZFC)),IIF(((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. SubStr(SM0->M0_CODMUN,1,2) == "31") .Or. ((cAliasSD2)->D2_TIPO=="I" .And. SF4->F4_AJUSTE == "S" .And. "RESSARCIMENTO" $ Upper(cNatOper) .And. "RESSARCIMENTO" $ Upper(cDescProd)),(cAliasSD2)->D2_TOTAL,0)),;    // customizado Sobel= retirado desconto
							IIF(SubStr(Alltrim((cAliasSD2)->D2_CF),1,1) == "7", "KG", SB1->B1_UM) ,;//retUn2UM( lNoImp2UM, cCFOPExp, Alltrim((cAliasSD2)->D2_CF), SB5->B5_UMDIPI, SB1->B1_UM ),;	// CUSTOMIOZAÇÃO SOBEL - PARA ATENDER NT 2016/001 DAS NOTAS DE EXPORTAÇÃO
							IIF(SubStr(Alltrim((cAliasSD2)->D2_CF),1,1) == "7", (cAliasSD2)->D2_XPESOEX, (cAliasSD2)->D2_QUANT),; 	//retQtd2UM( lNoImp2UM, cCFOPExp, Alltrim((cAliasSD2)->D2_CF), SB5->B5_CONVDIP, (cAliasSD2)->D2_QUANT ),;// CUSTOMIOZAÇÃO SOBEL - PARA ATENDER NT 2016/001 DAS NOTAS DE EXPORTAÇÃO
							(cAliasSD2)->D2_VALFRE,;
							(cAliasSD2)->D2_SEGURO,;
						    If(AllTrim(SC5->C5_MENPAD) == "053",nDesconto,0) ,;	//(/*nDesconto+nDescIcm+nDescRed*/0),;  //customizacao 1 Sobel-retirar o desconto  ||Customização 2 - 10/05/2018 //Caso for emitir nota fiscal de licitação, colocar o desconto para sair nesta posição ( nDesconto /*nDesconto+nDescIcm+nDescRed*/) ||
							...
							Do Case
								...
								Case AllTrim(CD2->CD2_IMP) == "IPI"
									If !lConsig
										...
										If ((cAliasSD2)->D2_TIPO == "D" .or. SF4->F4_PODER3 == "D") .and. (CD2->(FieldPos("CD2_PDEVOL")) > 0 .and. !Empty(CD2->CD2_PDEVOL))
											If cVerAmb >= "4.00" .and. lipiDev
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				If !Empty((cAliasSD1)->D1_NFORI)
					...
					Else
						...
						IF MsSeek (cMsSeek)
							...
							//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ							
							If cChave <> Dtos(SF2->F2_EMISSAO)+SD2->D2_SERIE+SD2->D2_DOC+SM0->M0_CGC+SM0->M0_ESTCOB+SF2->F2_ESPECIE+SF2->F2_CHVNFE							
								aadd(aNfVinc,{SD2->D2_EMISSAO,SD2->D2_SERIE,SD2->D2_DOC,SM0->M0_CGC,SM0->M0_ESTCOB,SF2->F2_ESPECIE,SF2->F2_CHVNFE,0,SD2->D2_PEDIDO})  
								lVinc := .T.
								cChave := Dtos(SF2->F2_EMISSAO)+SD2->D2_SERIE+SD2->D2_DOC+SM0->M0_CGC+SM0->M0_ESTCOB+SF2->F2_ESPECIE+SF2->F2_CHVNFE							
							EndIf							
		       ...
		       EndIf   		
				...
				aadd(aProd,	{Len(aProd)+1,;  
					...
					(cAliasSD1)->D1_VALFRE,;
					(cAliasSD1)->D1_SEGURO,;
						(/*(cAliasSD1)->D1_VALDESC + nDescRed*/0),; //Customizacao sobel
				   	IIF(!(cAliasSD1)->D1_TIPO$"IP",(cAliasSD1)->D1_VUNIT,0),;
				   	IIF(SB1->(FieldPos("B1_CODSIMP"))<>0,SB1->B1_CODSIMP,""),; //codigo ANP do combustivel
					IIF(SB1->(FieldPos("B1_CODIF"))<>0,SB1->B1_CODIF,""),; //CODIF  
					(cAliasSD1)->D1_LOTECTL,;//Controle de Lote
					(cAliasSD1)->D1_NUMLOTE,;//Numero do Lote 
					(cAliasSD1)->D1_DESPESA + (cAliasSD1)->D1_VALPS3 + (cAliasSD2)->D2_VALCF3 + nIcmsST + IIF( ((cAliasSD1)->D1_TIPO == "D" .And. !lEipiDev) .Or. ((cAliasSD1)->D1_TIPO == "B" .and. lIpiBenef),iif(cVerAmb >= "4.00" .And. (cAliasSD1)->D1_TIPO == "D" .And. !lEipiDev .and. cTPNota == "4" ,0,(cAliasSD1)->D1_VALIPI),0),;//Outras despesas + PISST + COFINSST  (Inclusão do valor de PIS ST e COFINS ST na tag vOutros - NT 2011/004).
					nRedBC,;//% Redução da Base de Cálculo
					cCST,;//Cód. Situação Tributária
...
If !Empty(aNota)
	...
	cString += NfeInfAd(cMensCli,cMensFis,aPedido,aExp,cAnfavea,aMotivoCont,aNota,aNfVinc,aProd,aDI,aNfVincRur,aRetido,cNfRefcup,cSerRefcupc,cTipo,nIPIConsig,nSTConsig,lBrinde,cVerAmb,Iif(aNota[5] == "D",aRefECF,{}),nVicmsDeson,nvFCPUFDest,nvICMSUFDest,nvICMSUFRemet,nvBCUFDest,aICMUFDest,nValIpiBene,npFCPUFDest,npICMSUFDest,npICMSInter,npICMSIntP,aObsCont)
	cString += "</infNFe>"
EndIf
...
//if SM0->M0_ESTCOB  ==  'RS'  // verifica se estado é RS 
...
//
// Alterado por TRS 23/04/2014  -sobel
//
//If !Empty(aIEST) 
...
//If dbSeek(xFilial("SA1") + aDest[01])
   If !Empty(SA1->A1_IESUBS)
      cString += '<IEST>'+ConvType(SA1->A1_IESUBS)+'</IEST>'
//    cString += NfeTag('<IEST>', SA1->A1_IESUBS)
   Endif   
Endif	                                      
RestArea(aAreaSA1)
RestArea(aArea)
// Fim da Alteracao por TRS
...
// --------------------------------------------------------
//      Customizacao TRS [ Ajuste de envio de EAN ] - Sobel 
//      Ajustes no envio do EANtrib e EAN         
// --------------------------------------------------------
DbSelectArea("SB1")
dbSetOrder(1)
If(DbSeek(xFilial("SB1")+aProd[02]))
	cString += '<ean>'+ConvType(SB1->B1_ZZCODBA)+'</ean>'
else
...
//Customização - Sobel
/* retirada a customização pois na NFE 4.0 se no ean tiver Gtin 14 nao pode ser igual no eantrib
DbSelectArea("SB1")
dbSetOrder(1)
If(DbSeek(xFilial("SB1")+aProd[02]))
	cString += '<eantrib>'+ConvType(SB1->B1_ZZCODBA)+'</eantrib>'
else
	cString += '<eantrib>'+IIF(!Empty(aProd[45]),ConvType(aProd[45]), ConvType(aProd[03]))+'</eantrib>'
endif    
*/    
cString += '<eantrib>'+IIF(!Empty(aProd[45]),ConvType(aProd[45]), ConvType(aProd[03]))+'</eantrib>'
...
Endif
cString += '<vUnTrib>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[12],21,8))+'</vUnTrib>'// estava 21/8 - Customização SOBEL	 em 03/09/18 voltei de 04 para 08
...
cVunCom := ConvType(aProd[16],21,4) // estava 21/8 - Customização SOBEL
cVunTrib:= ConvType(aProd[10]/aProd[12],21,4) // estava 21/8 - Customização SOBEL
	...
	Else	
		If Empty(cForma)
			If Len(aDupl) == 0
				cForma := "99"  //outros
			ElseIf Len(aDupl) > 0
				If aDupl[1][4] == "999"  // SOBEL CUSTOM FORMA DE PAGAMENTO
				 	cForma := "99"  // OUTROS
				 Else	
