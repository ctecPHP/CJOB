...
While !(cAliasQr1)->(EOF())
	...
	SFACAGRV()
	If lOK
		cGRVC5	:= " UPDATE T_PEDIDO_SOBEL SET NUMPEDIDOSOBEL = '"+cDoc+"' , DATAINTEGRACAOERP = '"+DtoS(dDataBase)+"' WHERE CODIGOUNIDFAT = '02' AND NUMPEDIDOAFV = '"+cNumAFV+"' "
		TCSqlExec(cGRVC5)
	EndIf
	(cAliasQr1)->(DbSkip())
EndDo
...
While !(cAliasQr1)->(EOF())
	...
	SFACAGRV()
	If lOK
		cGRVC5	:= " UPDATE T_PEDIDO_SOBEL SET NUMPEDIDOSOBEL = '"+cDoc+"' , DATAINTEGRACAOERP = '"+DtoS(dDataBase)+"' WHERE CODIGOUNIDFAT = '04' AND NUMPEDIDOAFV = '"+cNumAFV+"' " //elvis colocar um iff verificando se a variavel cDoc esta preenchida , se nao grava "K"
		TCSqlExec(cGRVC5)
	EndIf
	(cAliasQr1)->(DbSkip())
EndDo
